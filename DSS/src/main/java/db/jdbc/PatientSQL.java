...
public class PatientSQL implements PatientManager{
	...
	public void addPatient(PatientPojo patient) {
		...
		try {
			...
			String sql = "INSERT INTO Patient (id, name, gender, age /*, userId*/)";
			sql+= " VALUES (?,?,?,?,?,?,?)";
			PreparedStatement pstmt = DBManagerSQL.c.prepareStatement(sql); 
			
			pstmt.setInt(1, patient.getId());
			pstmt.setString(2,patient.getName().toString());
			pstmt.setString(3,patient.getGender().toString());
			pstmt.setInt(4,patient.getAge());
			//pstmt.setInt(5,patient.get);
